...
namespace Spegetti_Renderer
{
	namespace Graphics
	{
		...
		Post_Process_Effect Post_Process_Effect::operator=(Post_Process_Effect& other) const
		{
			if (this != &other)
			{
				Post_Process_Effect effect = Post_Process_Effect();
				effect.VAO = this->VAO;
				effect.VBO = this->VBO;
				effect.EBO = this->EBO;
				
				effect.Material_Effect = this->Material_Effect;
		...
		void Post_Process_Effect::Set_Up_Effect()
		{
		...
		{
			return this->Material_Effect;
		}


		void Post_Process_Effect::Draw()
		{
			this->Material_Effect->Use();
